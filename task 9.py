...
def create_emails(names, domains):
    for name in names:
        email = f"{name}.{random.randint(100,1000)}@{random.choice(string.ascii_lowercase) for _ in range(10)}.{random.choice(domains)}"
        print(email)
create_emails(names, domains)


# print(random.choice(string.ascii_lowercase)) for _ in range(10)